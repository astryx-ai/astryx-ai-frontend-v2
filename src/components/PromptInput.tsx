...
const PromptInput = ({ onSubmit, chatState, isThinking }: PromptInputProps) => {
  ...
  return (
    <div
      className={`sticky w-full bottom-0 z-10 backdrop-blur-lg  mx-auto ${chatState === ChatState.INITIAL ? "mt-10 max-w-3xl" : "mt-4 max-w-5xl"}`}
    >
      <div
        className={`bg-blue-astryx dark:bg-white relative rounded-lg mb-4 ${chatState === ChatState.INITIAL ? "h-[128px]" : "h-[78px]"} `}
      >
        <form
          className={`border absolute ${isFocused ? "bottom-2 right-2" : "bottom-0 right-0"} duration-200 w-full h-full p-4 rounded-lg bg-white border-blue-astryx dark:border-[#4b4b4b] dark:bg-black-85 shadow-white/10 hover:shadow-sm transition-all cursor-text ${chatState === ChatState.CHATTING ? "flex justify-between items-center " : null}`}
        ...
        >
          ...
          >
            ...
            {supported && chatState !== ChatState.INITIAL && chatState !== ChatState.THINKING && (
              <button
                onClick={e => {
                  e.preventDefault();
                  handleMicClick();
